...
class FormComponent(FormElement):
    ...
    def _process_required(self):
        if self.required and not any(v.get("name") in {"RelatedRequiredIfEqualValidator", "RequiredValidator", "ExactValidator", "RelatedUniversalRequiredValidator", "RelatedAnyOfValidator"} for v in self.validators):
